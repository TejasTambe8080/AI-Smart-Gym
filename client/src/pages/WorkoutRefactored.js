// Premium Unified Workout Interface - Professional Biometric Synchronization
...
const WorkoutRefactored = () => {
  ...
  const handlePoseDetected = (results) => {
    ...
    if (voiceEnabled && score < 70 && feedback.length > 0) voiceRef.current?.feedbackPosture(feedback[0]);
    if (repCounterRef.current.detect(landmarks)) {
      ...
      if (voiceEnabled) {
        ...
        if (count % 5 === 0) voiceRef.current?.motivate();
      }
    }
  };
  ...
  const endWorkout = async () => {
    ...
    try {
      ...
      setSummary({ workout: workoutData, performanceScore: performance, suggestions: PerformanceScorer.generateSuggestions({ ...workoutData, performanceScore: performance }) });
      localStorage.removeItem('active_workout_session');
      toast.success('Neural sync complete! 🚀');
    } catch (e) { 
       if (e.response?.status === 409) {
          toast.success('Sync complete (Sequence already recorded).');
        ...
        {!activeWorkout && !summary && (
          <>
            <div className="space-y-1">
              <h1 className="text-4xl font-black text-white italic tracking-tight uppercase">Protocol Selection</h1>
              <p className="text-slate-400 font-medium italic">Synchronize muscle group vectors for targeted biometrics calibration.</p>
            </div>
              ...
              <div className="space-y-8 animate-enter">
                 <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div className="flex items-center gap-4">
                       ...
                       <h2 className="text-2xl font-black text-white tracking-widest uppercase italic">{selectedMuscle} Base Sequences</h2>
                    </div>
                    {/* Search & Filters */}
                 ...
                 {loading ? (
                   <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                      {[1,2,3].map(i => <div key={i} className="h-48 skeleton rounded-[2rem]"></div>)}
                   </div>
                 ) : (
                   <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        ...
                        <div key={ex._id} className="premium-card p-8 group hover:bg-slate-800/40 border-slate-800 transition-all flex flex-col justify-between relative overflow-hidden">
                           ...
                           <div className="pt-8 flex items-center justify-between">
                              <div className="flex gap-4">
                                 <div>
                                    <p className="text-[10px] font-black uppercase text-slate-600">Unit Goal</p>
             ...
             <div className="flex flex-col lg:flex-row justify-between items-center gap-6">
                <div className="flex items-center gap-4">
                   ...
                   <div>
                      ...
                      <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest italic">Live Biometric Synchronization</p>
                   </div>
                </div>
                <button onClick={endWorkout} className="hidden lg:block h-14 px-10 bg-rose-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-rose-500 transition-all shadow-xl shadow-rose-500/20">Terminate Protocol</button>
             </div>
             ...
             <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
                <div className="lg:col-span-3 aspect-[3/4] md:aspect-video lg:h-[650px] bg-black rounded-[2rem] md:rounded-[3.5rem] overflow-hidden border-2 md:border-4 border-slate-800 shadow-2xl relative">
                   ...
                   <div className="absolute top-4 left-4 md:top-8 md:left-8 flex md:flex-col gap-2 md:gap-3 pointer-events-none">
                      ...
                      <div className="glass-card !bg-black/60 p-3 md:p-4 rounded-xl md:rounded-3xl border-white/10 backdrop-blur-xl">
                         <p className="text-[8px] md:text-[10px] font-black text-white/50 uppercase tracking-widest mb-0.5 md:mb-1">Units</p>
                   ...
                   <div className="absolute bottom-4 right-4 md:bottom-8 md:right-8 pointer-events-none">
                      <div className="glass-card !bg-black/60 p-4 md:p-10 rounded-2xl md:rounded-[4rem] border-white/5 backdrop-blur-3xl text-center">
                         <p className="text-[8px] md:text-[10px] font-black text-white/40 uppercase tracking-widest mb-0.5 md:mb-2">Neural Precision</p>
                ...
                <div className="space-y-6">
                   <div className="premium-card p-6 md:p-8 h-full flex flex-col gap-6">
                      <h4 className="text-[10px] font-black uppercase text-slate-500 tracking-[0.2em] italic border-b border-white/5 pb-4">Logic Constraints</h4>
                      <div className="flex-1 max-h-[150px] lg:max-h-none overflow-y-auto space-y-4">
                         ...
                         {postureFeedback.length === 0 && <div className="text-[10px] font-black text-emerald-500 uppercase italic tracking-tighter">[+] Symmetry Optimal</div>}
                      </div>
                      ...
                      <div className="flex flex-col gap-3">
                         <button onClick={() => setVoiceEnabled(!voiceEnabled)} className={`h-14 rounded-2xl text-[10px] font-black uppercase tracking-widest border transition-all ${voiceEnabled ? 'bg-blue-600 text-white border-blue-500 shadow-xl shadow-blue-500/20' : 'bg-slate-900 text-slate-500 border-slate-800'}`}>
                            Voice FEED: {voiceEnabled ? 'ACTIVE' : 'DEACTIVATED'}
                         </button>
                         <button onClick={endWorkout} className="lg:hidden h-14 bg-rose-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl shadow-rose-500/20">Terminate Protocol</button>
                      </div>
                   </div>
                </div>
             </div>
        ...
        {summary && (
          <div className="space-y-10 animate-enter">
             <div className="text-center space-y-2">
                ...
                <h2 className="text-5xl font-black text-white tracking-widest uppercase italic">Neural Sync Complete</h2>
                <p className="text-slate-400 font-medium italic">Performance matrix categorized for growth prediction.</p>
             </div>

             <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
                {[
                  { l: 'Energy Units', v: summary.workout.reps, c: 'blue' },
                  { l: 'Protocol Flow', v: formatTime(summary.workout.duration), c: 'emerald' },
                  { l: 'Precision Mean', v: `${summary.workout.postureScore}%`, c: 'amber' },
                  { l: 'Performance Index', v: `${summary.performanceScore}%`, c: 'rose' }
                ].map((s, i) => (
             ...
             <div className="grid lg:grid-cols-2 gap-10 pt-10">
                <div className="premium-card p-10 bg-gradient-to-br from-blue-600/10 to-indigo-600/10 border-blue-500/20">
                   <h4 className="text-xl font-black text-white italic uppercase mb-6 tracking-tight">AI Strategy Infusion</h4>
                   <div className="space-y-6">
                ...
                <div className="flex flex-col gap-4 justify-center">
                   <button onClick={() => navigate('/dashboard')} className="btn-primary h-20 text-lg uppercase italic tracking-widest">Store and Exit</button>
                   <button onClick={() => { setSelectedExercise(null); setSummary(null); }} className="btn-secondary h-20 text-lg uppercase italic tracking-widest">Execute New Protocol</button>
                </div>
             </div>
          </div>
        )}
